...
public class NacosPropertySourceLocator implements PropertySourceLocator {
	...
	public PropertySource<?> locate(Environment env) {
		...
		String dataIdPrefix = nacosConfigProperties.getPrefix(); // 先从spring.cloud.nacos.config.prefix取
		if (StringUtils.isEmpty(dataIdPrefix)) {
			dataIdPrefix = name;
		}

		if (StringUtils.isEmpty(dataIdPrefix)) { // 如果spring.cloud.nacos.config.prefix未指定  再从spring.application.name取
	...
	private void loadApplicationConfiguration(
		...
		String fileExtension = properties.getFileExtension(); // 后缀名 yaml
		String nacosGroup = properties.getGroup(); // 组  shr-group
		// load directly once by default  优先级最高-2   application
		loadNacosDataIfPresent(compositePropertySource, dataIdPrefix, nacosGroup,
				fileExtension, true);
		// load with suffix, which have a higher priority than the default
		loadNacosDataIfPresent(compositePropertySource,  // 优先级最高-1    application.yaml
				dataIdPrefix + DOT + fileExtension, nacosGroup, fileExtension, true);
		// Loaded with profile, which have a higher priority than the suffix
		for (String profile : environment.getActiveProfiles()) {   // 优先级最高
			String dataId = dataIdPrefix + SEP1 + profile + DOT + fileExtension; // 比如 application-dev.yaml  profiles有多个
